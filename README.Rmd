...
[neuPrint website](https://neuprint.janelia.org/help/videos?dataset=hemibrain) and accessed programmatically in R using [neuprintr](https://github.com/natverse/neuprintr). This tool also will match the registered data to the corresponding neuron in the hemibrain connectome. 

# How have these registrations been performed? 
In the past, light-level microscopy data could be registrated using the [fiji-cmtk-gui](https://github.com/jefferis/fiji-cmtk-gui). This tool automates this process more without haveing to open the GUI and also adds functionality by co-plotting these neurons with connectome date. 

In the following, we detail some of `nat-tech`'s functionality 

# Example Functions
```{r cars, echo = TRUE, eval = FALSE, include = TRUE}
...
To run the pipeline
```{r pressure, echo = FALSE, eval = TRUE, include = TRUE}
...
This package was created by Emily Kellogg and Alexander Bates while in the group of [Dr. Rachel Wilson](https://en.wikipedia.org/wiki/Rachel_Wilson_(neurobiologist)). 

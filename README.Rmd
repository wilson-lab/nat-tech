...
![regimages](inst/images/registered_images.png) The next thing that will happen is that the code will get the specified neuron from the hemibrain connectome and then register this to the specified template brain and convert it into a .nrrd file.

```{r echo = TRUE, eval = FALSE, include = TRUE}
hemibrain_to_nrrd()
```

This is what that process looks like ![regimages2](inst/images/hemibrain_to_nrrd.png) Finally, the code will run a FIJI macro to combine your reformatted images with the specified neuron from the hemibrain that got converted into a .nrrd file.
